...
inline float4 GetUnderwaterColorAlbedo(float2 uv, float3 albedoColor, float3 vertexWorldPos)
{
		...
		float3 volLight = GetVolumetricLightWithAbsorbtion(uv, uv, transparent, turbidityColor, dyeColor, 0, distanceToVertex,  waterID, GetExposure(), albedoColor * 5).xyz;
...
inline float4 GetUnderwaterColorRefraction(float2 uv, float3 albedoColor, float2 refractionNormal, float3 vertexWorldPos)
{
		...
		float3 volLight = GetVolumetricLightWithAbsorbtion(uv, uv, transparent, turbidityColor, dyeColor, refraction, distanceToVertex, waterID, GetExposure(), albedoColor * 5).xyz;
		return half4(volLight, 1);
