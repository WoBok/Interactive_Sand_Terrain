...
inline float GetMaxRayDistanceRelativeToTransparent(uint waterID)
{ 
	//return KWS_MAX_TRANSPARENT;
...
half RaymarchCaustic(RaymarchData raymarchData, float3 currentPos, float3 lightForward)
{
	...
	float offsetLength = (raymarchData.waterHeight - currentPos.y - vericalOffset) / angle;
...
RaymarchData InitRaymarchData(vertexOutput i, float waterMask)
{
	...
	data.rayLength = GetMaxRayDistanceRelativeToTransparent(waterID);
